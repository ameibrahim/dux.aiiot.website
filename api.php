...
function signup($conn, $name, $email, $phone, $stdNumber, $address, $password, $department, $photoName)
{
    ...
    $sql = "INSERT INTO users (`uid`, `name`, `email`, `password`, `phone`, `stdnumber`, `address`, `department`, `status`, `photo`) 
            VALUES ('$uid', '$name', '$email', '$password', '$phone', '$stdNumber', '$address', '$department', '$status', 'uploads/'.'$photoName')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $allowedDomains = array('dux.aiiot.website', 'localhost', 'localhost:8888', 'dev.local:8888');
    ...
    if ($action === 'login') {
        ...
        signup($conn, $name, $email, $phone, $stdNumber, $address, $password, $department, $photoName);
...
// fetching courses generally for the admin's dropdown:
    ...
    function getCoursesTeacher($conn, $teacherId) {
        ...
        $query = "SELECT courseId, courseName FROM courses WHERE createdBy = '$teacherId'";
...
function getStudentsRequestList($conn, $myId, $courseId ) {
    ...
    INNER JOIN courses ON subscriptions.courseId = courses.courseId WHERE courses.courseId = '$courseId' AND courses.createdBy ='$myId'";
